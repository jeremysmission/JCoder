...
| 17B | Beast: vLLM full-stack bring-up with dual RTX 3090 | NEXT | vLLM serving Qwen3-Coder 80B with tensor parallelism across both GPUs, embedder and reranker co-located, latency baselined |
...
| 18B | Beast: Online distillation from GPT-4o to local Qwen3 | PLANNED | Distillation dataset generated via GPT-4o, fine-tuned local model measured against base, cost-vs-quality tradeoff documented |
...
| 25 | Beast: Supercomputer burst pipeline (local, no cloud needed) | PLANNED | Beast's dual RTX 3090 + 128 GB RAM runs weekend burst locally instead of cloud. Parallel evolution experiments across GPU-partitioned vLLM instances. |
...
### Sprint 17B: Beast vLLM Full-Stack Bring-Up (NEW -- Beast-Enabled)

Beast unlocks running Qwen3-Coder-Next 80B with tensor parallelism across dual RTX 3090 (48 GB combined VRAM). This was impossible on Toaster.

1. `17B.1` Install vLLM with CUDA support, verify both GPUs visible via `nvidia-smi` and `torch.cuda.device_count()`.
2. `17B.2` Launch Qwen3-Coder 80B with `--tensor-parallel-size 2` on port 8000. Verify generation works end-to-end.
3. `17B.3` Launch Nomic Embed Code 7B on port 8001 and Qwen3-Reranker-4B on port 8002. Verify co-location within VRAM budget.
4. `17B.4` Run full `doctor` + `measure` against the three-endpoint stack. Capture latency baseline (P50, P95, P99).
5. `17B.5` Run `ask` and `agent run` against the live 80B model. Compare answer quality against previous smaller-model baselines.
6. `17B.6` Run 4-hour soak test with sustained query load. Document throughput ceiling and thermal behavior.
...
### Sprint 18B: Beast Online Distillation From GPT-4o To Local Qwen3 (NEW -- Beast-Enabled)

Beast enables the full distillation pipeline: generate high-quality training data via GPT-4o API, fine-tune the local Qwen3-Coder model on Beast's dual GPUs.

1. `18B.1` Generate distillation dataset: send 500+ coding tasks to GPT-4o, capture structured input/output pairs with chain-of-thought reasoning.
2. `18B.2` Format distillation data for vLLM fine-tuning (conversation format, quality filtering, deduplication).
3. `18B.3` Run LoRA fine-tune of Qwen3-Coder on Beast using the distillation dataset. Track loss curves and checkpoint quality.
4. `18B.4` Evaluate distilled model on JCoder's 200-question eval set. Compare against base Qwen3 and GPT-4o on accuracy, latency, and cost.
5. `18B.5` Document cost-vs-quality tradeoff. Decide whether distilled model replaces base for daily use.

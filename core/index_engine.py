...
class IndexEngine:
    ...
    def __init__(
        ...
        if faiss is None:
            ...
            log.warning("faiss not installed — dense vector search disabled, using FTS5-only")
            return
        ...
        if gpu_count > 0 and hasattr(faiss, "index_cpu_to_all_gpus"):
            ...
            if free_mb > gpu_safety_margin_mb:
                ...
                log.warning("FAISS GPU skipped: %s — using CPU", reason)
                self.index = self._flat_index
        else:
            self.index = self._flat_index
    ...
    def search_keywords(self, query: str, k: int) -> List[Tuple[int, float]]:
        ...
        try:
            ...
            if not self._fts5_error_logged:
                log.warning("FTS5 query error (logged once): %s", e)
        ...
        if faiss is not None and os.path.exists(faiss_path):
            ...
            if self._gpu_available:
                try:
                    ...
                    log.warning("FAISS GPU reload failed: %s — using CPU", e)
